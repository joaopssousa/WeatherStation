...
static void TEST_PROTECTIONS_RunPCROP(void)
{
  /*No Errors detected: this is normal as this platform does not support PCROP */
...
static void TEST_PROTECTIONS_RunWRP(void)
{
  ...
  printf("  -- Reading 0x%lx bytes at address: 0x%lx (for backup)\r\n\n", TEST_PROTECTIONS_WRP_FLASH_SIZE,
  ...
  /* 2 - Erasing page */
  printf("  -- Erasing 0x%lx bytes at address: 0x%lx\r\n\n", TEST_PROTECTIONS_WRP_FLASH_SIZE,
  TEST_PROTECTIONS_WRP_FLASH_ADDRESS);
  ...
  else
  {
    ...
    {
      printf("  -- Written successfully at address: 0x%lx\r\n\n", TEST_PROTECTIONS_WRP_FLASH_ADDRESS);
  ...
  while (exit == 0U)
  {
    ...
    if (COM_Receive(&key, 1U, RX_TIMEOUT) == HAL_OK)
    {
      ...
      if (exit != 1U)
      {
        ...
        {
          printf("SLOT_ACTIVE_%ld is not configured !\r", slot_number);
        }
        else
        {
...
static void TEST_PROTECTIONS_RunIWDG(void)
{
  ...
  printf("  -- Waiting %ld (ms). Should reset if IWDG is enabled. \r\n\n", TEST_PROTECTIONS_IWDG_DELAY);
